...
void MinHeapifyDown(int now)   // go down from pos
{
    ...
    for(;;) // go down       index of the smaller value go up
    {
        ...
        if(ValueOfIndex[HeapOfIndex[l]] == INT_MAX &&
           ValueOfIndex[HeapOfIndex[r]] == INT_MAX) break; // reach bottom
        if(ValueOfIndex[HeapOfIndex[now]] <= ValueOfIndex[HeapOfIndex[l]] &&
           ValueOfIndex[HeapOfIndex[now]] <= ValueOfIndex[HeapOfIndex[r]]) break;
...
int HeapExtractIndexOfMinValue()
{
    ...
    -- HeapSize;
    if(HeapSize > 1) MinHeapifyDown(1);
    return MinIndex;
}
