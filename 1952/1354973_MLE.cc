#include<iostream>
#include<vector>
#include<list>
using namespace std;

int i, j, n, maxv;
int g[5000];
int maxlen[5000];
vector<int> next[5000];
list<vector<int> > toUnique;

void load(int start, vector<int> v)
{
    v.push_back(g[start]);
    if(next[start].size() == 0) toUnique.push_back(v);
    else
    {
        for(int i = 0; i < next[start].size(); ++ i)
        {
            load(next[start][i], v);
        }
    }
}

void unique()
{
    list<vector<int> >::iterator itr = toUnique.begin(), p;
    for(; itr != toUnique.end(); ++ itr)
    {
        p = itr;
        ++ p;
        for(; p != toUnique.end();)
        {
            if(*p == *itr) p = toUnique.erase(p);
            else ++ p;
        }
    }
}

int main()
{
    scanf("%d", &n);
    for(i = 0; i < n; ++ i) scanf("%d", &g[i]);
    for(i = n - 1; i >= 0; -- i)
    {
        maxv = 0;
        vector<int> can;
        for(j = i; j < n; ++ j)
        {
            if(g[j] < g[i])
            {
                if(maxlen[j] > maxv)
                {
                    can.clear();
                    can.push_back(j);
                    maxv = maxlen[j];
                }
                else if(maxlen[j] == maxv) can.push_back(j);
            }
        }
        next[i] = can;
        maxlen[i] = maxv + 1;
    }
    vector<int> gg;
    maxv = -1;
    for(i = 0; i < n; ++ i)
    {
       if(maxlen[i] > maxv)
       {
           gg.clear();
           gg.push_back(i);
           maxv = maxlen[i];
       }
       else if(maxlen[i] == maxv) gg.push_back(i);
    }
    for(i = 0; i < gg.size(); ++ i) load(i, vector<int>());
    unique();
    printf("%d %d", maxv, toUnique.size());
    return 0;
}